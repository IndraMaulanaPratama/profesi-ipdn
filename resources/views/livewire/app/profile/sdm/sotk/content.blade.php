{{-- Be like water. --}}

<div class="my-5 container">
    <div class="row justify-content-center my-4">
        <div class="col-6">
            <div class="row p-3 justify-content-between">

                {{-- Dr. Drs. H. Endang Try Setyasih, MM --}}
                <div class="col-6 text-center">
                    <img src="{{ asset('assets/homepage/images/Dr. Drs. H. Endang Try Setyasih, MM.png') }}"
                        class="rounded" width="90%" alt="" srcset="">

                    <p class="text-center pt-4" style="font-weight: bold; font-size: 130%">
                        Dr. Drs. H. Endang Try Setyasih, MM
                    </p>
                    <p style="color: #063A69; font-size: 16px; font-weight: 700">
                        Direktur
                    </p>

                </div>

                {{-- Drs. Komaedi, M.Si --}}
                <div class="col-6 text-center">
                    <img src="{{ asset('assets/homepage/images/Drs. Komaedi, M.Si.png') }}" class="rounded" width="90%"
                        alt="" srcset="">

                    <p class="text-center pt-4" style="font-weight: bold; font-size: 130%">
                        Drs. Komaedi, M.Si
                    </p>

                    <p style="color: #063A69; font-size: 16px; font-weight: 700">
                        Sekertaris Direktur
                    </p>

                </div>

            </div>
        </div>
    </div>

    <div class="row justify-content-center">
        <div class="col-9">
            <div class="row p-3 justify-content-between">

                {{-- hardiyanto_rahman --}}
                <div class="col-3 text-center">
                    <img src="{{ asset('assets/homepage/images/sotk_hardiyanto_rahman.png') }}" class="rounded" height="250px"
                        alt="" srcset="">

                    <p class="text-center pt-4" style="font-weight: bold; font-size: 130%">
                        Hardiyanto Rahman, S.IP, M.Si
                    </p>

                    <p style="color: #063A69; font-size: 16px; font-weight: 700">
                        Kepala Unit Laboratorium Pemerintahan, Pangakalan Data dan Sistem Informasi
                    </p>

                </div>

                <div class="col-3 rounded text-center">
                    <img src="{{ asset('assets/homepage/images/Group-115.png') }}" class="rounded" height="250px"
                        alt="" srcset="" class="round">

                    <p class="text-center pt-4" style="font-weight: bold; font-size: 130%">
                        Fransina Makdalena Paulla Nusmesse, S.Sos, M.Si
                    </p>

                    <p style="color: #063A69; font-size: 16px; font-weight: 700">
                        Kepala Unit Penjaminan Mutu
                    </p>

                </div>

                <div class="col-3 text-center">
                    <img src="{{ asset('assets/homepage/images/Group-115.png') }}" class="rounded" height="250px"
                        alt="" srcset="">

                    <p class="text-center pt-4" style="font-weight: bold; font-size: 130%">
                        Cut Laila Karmila, S.Sos, M.A
                    </p>

                    <p style="color: #063A69; font-size: 16px; font-weight: 700">
                        Kepala Unit Perpustakaan
                    </p>

                </div>
            </div>

            <div class="row p-3 justify-content-between">

                <div class="col-3 text-center">
                    &nbsp;
                </div>

                <div class="col-3 text-center">
                    <img src="{{ asset('assets/homepage/images/Group-115.png') }}" class="rounded" height="250px"
                        alt="" srcset="">

                    <p class="text-center pt-4" style="font-weight: bold; font-size: 130%">
                        Muhammad Hanif Nashrullah, S.STP
                    </p>

                    <p style="color: #063A69; font-size: 16px; font-weight: 700">
                        Kepala Unit Penjaminan Mutu
                    </p>

                </div>

                <div class="col-3 text-center">
                    &nbsp;
                </div>
            </div>
        </div>
    </div>
</div>
